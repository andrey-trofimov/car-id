...
async function checkQr() {
    ...
    const fetchUrl = `https://car-id.ru/api/qr/${qrId}`;
...
async function createMsg() {
    try {
        const fetchUrl = "https://car-id.ru/api/report/createDraft";
        ...
        if (response.ok) {
            ...
            if (![undefined, null, ""].includes(response.notification_id)) {
                ...
                if (window.location.host.includes("car-id.ru")) notificationId = response.notification_id;
