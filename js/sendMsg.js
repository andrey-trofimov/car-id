...
let carEventList = [];
let timeoutId = 0;
const notificationId = window.location.host.includes("car-id.ru")
    ? window.location.pathname.slice(14, window.location.pathname.length - 1)
    : sessionStorage.getItem("notificationId");
...
async function sendMsg(eventId) {
    const fetchUrl = "https://car-id.ru/api/report/updateDraft";
